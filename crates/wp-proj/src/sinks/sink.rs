...
impl Sinks {
    ...
    fn ensure_defaults_file(sink_root: &std::path::Path) -> RunResult<()> {
        ...
        let should_write = if p.exists() {
            match std::fs::read_to_string(&p) {
                Ok(body) => {
                    // Overwrite empty files or files generated by old templates
                    // that start with `version = "2.0"` (rejected by strict schema).
                    body.trim().is_empty() || body.contains("version = \"2.0\"")
                }
                Err(_) => true,
            }
        } else {
    ...
    fn ensure_business_demo(sink_root: &std::path::Path) -> RunResult<()> {
        ...
        let should_write = if demo.exists() {
            match std::fs::read_to_string(&demo) {
                Ok(content) => content.contains("version = \"2.0\""),
                Err(_) => true,
            }
        } else {
            true
