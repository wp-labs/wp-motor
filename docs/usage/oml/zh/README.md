...
- ⭐ **新增** `static { ... }` 块：一次构造、多处复用，彻底替代旧的 `__temp` 模板缓存
- ⭐ **新增** `static` vs `__temp` 性能基准（`cargo bench -p wp-oml --bench oml_static_block`），量化优化收益
- 📖 **文档**：补充《[static 块常量与模板缓存](./static_blocks.md)》，说明语法、执行模型与注意事项
