...
- **OML 加载错误提示过泛**：修复 `load-engine-res` 阶段 OML 解析失败时控制台仅显示 "配置错误"、无具体原因的问题——`conv_err()` 把 OML 语法详情压缩为无 detail 的配置错误；现保留 `OMLCodeReason::Syntax` 内层消息作为 detail，错误提示定位到具体文件与解析失败位置
