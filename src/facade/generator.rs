    ...
    while produced < total {
        ...
        for ffv in rows {
            ...
            wp_connector_api::AsyncRawDataSink::sink_str(&mut sink, &raw_line)
                ...
                .owe_sink()
                .with("rule_gen_run")
                .doing("write generated rule line to sink")?; // map sink error into RunError(Dist)
            produced += 1;
        }
    }
    Ok(())
...
pub async fn sample_gen_run(
    ...
    for f in files {
        let file = std::fs::File::open(&f)
            .into_as(wp_error::RunReason::from_conf(), "open sample file failed")
            .with(&f)
    ...
    for s in samples.iter().cycle().take(total) {
        AsyncRawDataSink::sink_str(&mut sink, s)
            ...
            .owe_sink()
            .with("sample_gen_run")
            .doing("write generated sample line to sink")?; // map sink error into RunError(Dist)
    }
    Ok(())
}
